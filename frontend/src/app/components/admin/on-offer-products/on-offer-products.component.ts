...
@Component({
  selector: 'app-on-offer-products',
  standalone: true,
  imports: [CommonModule, NotificationsComponent],
  templateUrl: './on-offer-products.component.html',
  styleUrl: './on-offer-products.component.css'
})
export class OnOfferProductsComponent implements OnChanges, OnInit {
  ...
  getProducts() {
    this.ps.getAllProducts().subscribe({
      next: (response) => {
        if (response.success) {
          this.products = (response.products as Product[]).filter(p => p.OnOffer);
        } else {
          this.ns.showMessage(response.error as string, false);
        }
      },
